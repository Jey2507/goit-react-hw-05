        ...
        <div>
            ...
            <div className={css.button}>
                <Link to={backLinkURLRef.current} className={css.linkDescr}>
                  <p className={css.back}><FaArrowLeft />Go back</p></Link>
            </div>
            {movie && (
              <div className={css.container}>
                <div className={css.divLinks}>
                  <img className={css.image} src={`https://image.tmdb.org/t/p/w500${movie.poster_path}`} alt={movie.overview} />
                  <div className={css.divDescr}>
                    <div>
                      <h1 className={css.title}>{`${movie.title} (${movie.release_date.slice(0,4)})`}</h1>
                      <p className={css.userScore}>User score: {(movie.vote_average * 10).toFixed() + "%"}</p>
                      <h2 className={css.overview}>Overview</h2>
                      <p className={css.descr}>{movie.overview}</p>
                      <h2 className={css.overview}>Genres</h2>
                      <ul className={css.list}>
                        {movie.genres.map(genre => (<li className={css.descr} key={genre.id}>{genre.name}</li>))}
                      </ul>
                  </div>
                  ...
                  <div className={css.infoDiv}>
                    ...
                    <nav className={css.navLink}>
                      <NavLink className={getNavLinkClass} to="cast">Cast</NavLink>
